    ...
    let
      ...
      perSystem = system:
        let
          ...
          # Name and version come from Cargo.toml so a release only ever has
          # to touch one file — a hardcoded copy here silently disagrees with
          # `--version` the first time someone forgets it.
